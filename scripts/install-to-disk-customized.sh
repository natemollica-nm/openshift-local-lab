...
if [ ! -f coreos-installer.done ]; then
  ...
  echo "Executing coreos-installer with the following options: install -i /opt/openshift/master.ign /dev/vda"
  coreos-installer install -n -i /opt/openshift/master.ign /dev/vda
